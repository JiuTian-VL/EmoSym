...
class FI(Dataset):
    def __init__(self, root_dir, min_pixels, max_pixels, processor, transform=None, split='train'):
        """ 
        Args:
    ...
    def __getitem__(self, idx):
        ...
        prompt = 'Describe the most emotionally reflective factor in the image in one sentence, using up to 30 words.'
...
class DataCollatorForAugmentDataset(object):
    ...
    def __call__(self, examples):
        ...
        batch_annotation_text = [
            example['annotation_path']
        ...
        data_dict = {
            ...
            'batch_annotation_text': batch_annotation_text
        }
...
def make_pretrain_data_module_FI(processor, data_args):
    sft_dataset = FI(
        root_dir='/data4/Zyj_MM/FI/image', min_pixels=data_args.min_pixels, max_pixels=data_args.max_pixels, processor=processor, context=data_args.context
    )
...
class EmoSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        prompt = 'Describe the most emotionally reflective factor in the image in one sentence, using up to 30 words.'
        data.update({'annotation_path': annotation_path})
...
def make_augment_data_module_EmoSet(processor, data_args):
    sft_dataset = EmoSet(
        data_root='/data4/Zyj_MM/EmoSet_aug_online_bank', num_emotion_classes=8, phase='train', 
        min_pixels=data_args.min_pixels, max_pixels=data_args.max_pixels, processor=processor, context=data_args.context
    )
    
    data_collator = DataCollatorForAugmentDataset(processor)
