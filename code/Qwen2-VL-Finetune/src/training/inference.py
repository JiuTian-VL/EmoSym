...
def inference(arg, emotion):
    ...
    mapper_context = model_dict[arg.mapper_name](arg)
    state = torch.load(os.path.join(working_path, "mapper_context.pth"))
    mapper_context.load_state_dict(state)
    mapper_context.eval()
    mapper_context.to(device)


    e_mean = torch.load(f"emotion_space/{emotion}_cls_fea_mean.pt")
    e_std = torch.load(f"emotion_space/{emotion}_cls_fea_std.pt")
    e_mean_context = torch.load(f"emotion_space/{emotion}_context_mean.pt")
    e_std_context = torch.load(f"emotion_space/{emotion}_context_std.pt")


    normal = torch.distributions.Normal(e_mean, e_std)
    normal_context = torch.distributions.Normal(e_mean_context, e_std_context)
    ...
    for _ in range(num_picture):
        ...
        e_context = normal_context.sample((1,))
      
        # pdb.set_trace()
        
        # pred_emd_emo = mapper(emo_v) 
        pred_emd_emo = mapper(e_vec.to(device)) 
        pred_emd_context = mapper_context(e_context.to(device))
        
        
        pred_emd = pred_emd_emo + pred_emd_context*0.01
        # pred_emd = pred_emd_emo + attr_rate_scene * pred_emd_scene
        # pred_emd = pred_emd_emo + pred_emd_scene + pred_emd_object

        # token_embeds[token_id] = pred_emd_emo
...
def generate(cur_dir, device,model, num_fc_layers=1, need_LN=False, need_ReLU=False, need_Dropout=False, use_prompt=False):
    ...
    parser.add_argument("--emotion_idx", type=int, default=None, help="emotion idx.")
