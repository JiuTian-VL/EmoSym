...
def configure_llm(model, training_args):
    ...
    if training_args.context:
        clip_params = model.text_encoder.parameters()
        set_requires_grad(clip_params, False)
...
def train():
    ...
    model = Qwen2VLForConditionalGeneration.from_pretrained(
        ...
        context=training_args.context,
        training_context=True if training_args.context else False,
        **bnb_model_from_pretrained_args
    )
    ...
    if training_args.lora_enable:
        ...
        # set_requires_grad(model.lm_head.parameters(), True)
        set_requires_grad(model.cls_head.parameters(), True)
        model.EMO_embed.requires_grad = True
        if training_args.context:
            set_requires_grad(model.con_head.parameters(), True)
            set_requires_grad(model.text_encoder.parameters(), False)
